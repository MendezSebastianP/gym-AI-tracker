...
- **Gamification:** XP per session (base 50, scaled by history), PRs trigger bonus XP. Level-up costs `level * 100` XP. Shop (`pages/Shop.tsx`) sells themes and streak skins (`/gamification/shop`).
- **Streak integrity:** `sessions.streak_eligible_at` is set once on first completion (`datetime.now(UTC)`) and never overwritten — all streak calculations use this field, not the user-editable `completed_at`. Migration `d1e2f3a4b5c6` adds the column with backfill. Streak coins are **not** auto-awarded on session completion; they accumulate as unclaimed weeks and are claimed manually via `POST /gamification/streak/claim` (batch, historically-correct tier coins). Frontend prefers server-side `streak_slots` from `/gamification/stats` over local Dexie data for both the flame row and streak count.
- **Streak skins:** 6 visual variants (skin_a–skin_d) defined in `components/StreakFlames.tsx`. `SKIN_ACCENTS` and `SKIN_CLAIM_CFG` drive per-skin colors in Stats.tsx. Preview at `/playground` (admin-only).
