...
function FeedCard({ sessionId, isTarget, allRoutines }: {
    ...
    useEffect(() => {
        if (!routine || !session || session.day_index === undefined) return;
                    ...
                    return (
                        <div key={i} style={{
                            ...
                            <div style={{ fontWeight: 600, marginBottom: '6px', display: 'flex', alignItems: 'center', gap: '6px' }}>
                                {ex.name}
                                {ex.is_bodyweight && (
                                    <span style={{ fontSize: '10px', background: 'var(--bg-secondary)', padding: '1px 5px', borderRadius: '4px', color: 'var(--text-tertiary)' }}>BW</span>
                                )}
                            </div>
